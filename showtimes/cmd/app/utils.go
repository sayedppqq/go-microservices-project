...
import (
	...
	"flag"
	"fmt"
	"github.com/sayedppqq/go-microservices-project/showtimes/pkg/models/mongodb"
...
func (opt *connectionOptions) GetApplication(errorLog, infoLog *log.Logger, client *mongo.Client) *application {
	...
	return &application{
		...
		infoLog:  infoLog,
		showtimes: &mongodb.ShowTimeModel{
			C: client.Database(*opt.mongoDatabase).Collection("showtimes"),
		},
	}
}
...
func (opt *connectionOptions) pasrseCommandLineFlags() {
	...
	opt.serverPort = flag.Int("serverPort", 4002, "HTTP server network port")
	opt.mongoURI = flag.String("mongoURI", "mongodb://localhost:27017", "Database hostname url")
	opt.mongoDatabase = flag.String("mongoDatabase", "showtimes", "Database name")

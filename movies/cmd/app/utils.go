...
import (
	...
	"flag"
	"fmt"
	"github.com/sayedppqq/go-microservices-project/movies/pkg/models/mongodb"
...
func (opt *connectionOptions) GetApplication(errorLog, infoLog *log.Logger, client *mongo.Client) *application {
	...
	return &application{
		...
		infoLog:  infoLog,
		movies: &mongodb.MovieModel{
			C: client.Database(*opt.mongoDatabase).Collection("movies"),
		},
	}
}
...
func (opt *connectionOptions) pasrseCommandLineFlags() {
	...
	opt.serverPort = flag.Int("serverPort", 4001, "HTTP server network port")
